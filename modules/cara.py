...
class classifier_hidden(nn.Module):
    def __init__(self, hidden_size, nz):
        super(classifier_hidden, self).__init__()
        self.linear1 = nn.Linear(hidden_size, 10)
        self.linear2 = nn.Linear(10, nz)
...
class CARA(nn.Module):
    def __init__(self, encoder, decoder, tokenizer_encoder, tokenizer_decoder, args): #
        ...
        self.latent_classifier = classifier_hidden(self.nz, args.label_size if args.label_size > 2 else 1)
        ...
        self.classifier = classifier_hidden(self.encoder.config.hidden_size, 1 if args.label_size <= 2 else args.label_size)
    ...
    def forward(self, input_seq_ids, tgt_seq_ids, cond_labels, attention_mask):
        ...
        #print(prob_gen_z_dis, zeros_label.squeeze())
        loss_lsd = self.BCEWithLogitsLoss(prob_gen_z_dis, zeros_label.squeeze()) + self.BCEWithLogitsLoss(prob_encode_z_dis, ones_label.squeeze())
        acc_encode_z_dis = ((prob_encode_z_dis >= 0).float() == ones_label.squeeze()).float()
        acc_gen_z_dis = ((prob_gen_z_dis >= 0).float() == zeros_label.squeeze()).float()
        # Train sampler adversarially
        loss_lsg = self.BCEWithLogitsLoss(prob_gen_z_dis, ones_label.squeeze())

        ####################  Latent classifier for disentanglement ###################sa
        ...
        if self.args.label_size <= 2:
            ...
            # Train encoder adversarially
            loss_encoder = 1 - self.CrossEntropyLoss(prob_encode_z_cls, cond_labels.squeeze())


        #################### Recontruction loss with latent z and label emb #################### 
        # Embed labels
        label_emb = self.label_embedding(cond_labels)  # (B, hidden_size)
        # past_label = self.decoder.linear(label_emb)    # (B, n_blocks * hidden_size)  # : use the same linear layer for latent_z for now.
        if self.args.label_size <= 2:
            sampled_cond_labels = 1 - cond_labels
        else:
            sampled_cond_labels = torch.zeros(cond_labels.size()).to(device=input_seq_ids.device, dtype=torch.long)
            
            for i in range(len(cond_labels)):
                while True:
                    rand = np.random.randint(self.args.label_size)
                    if rand!= int(cond_labels[i]):
                        sampled_cond_labels[i] = rand
                        break

            sampled_cond_labels = sampled_cond_labels.type(torch.long).to(device=input_seq_ids.device, dtype=torch.long)
        ...
        past = latent_z + label_emb.squeeze() # (B, n_blocks * hidden_size)
        #print(latent_z.size(), label_emb.squeeze().size(), past.size())
        #print(past, latent_z)
        ...
        loss = loss_rec + loss_latent_space
    ...
    def control_generation(self, input_seq_ids, cond_labels, attention_mask):
        ones_label = torch.ones_like(cond_labels).to(device=input_seq_ids.device, dtype=torch.float32)
        zeros_label = torch.zeros_like(cond_labels).to(device=input_seq_ids.device, dtype=torch.float32)
        random_noise = torch.nn.init.normal_(torch.empty(input_seq_ids.size(0), self.nz)).to(device=input_seq_ids.device, dtype=torch.float32)
        ...
        past = latent_z + label_emb.squeeze()
